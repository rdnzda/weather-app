...
import { getCoordinatesFromCity, getWeather, getForecast } from "../services/api";
...
function Dashboard({ selectedCity }) {
  ...
  const fetchWeather = async (cityName) => {
    try {
      setLoading(true);
      const { lat, lon, name, country } = await getCoordinatesFromCity(cityName);
      const weather = await getWeather(lat, lon);
      const forecastData = await getForecast(lat, lon);
  ...
  useEffect(() => {
  if (selectedCity) {
    // Si selectedCity est défini, charger directement sa météo
